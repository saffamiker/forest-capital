...
| Layer | Tests | Notes |
|-------|-------|-------|
| Backend (pytest) | 864 passed, 10 skipped | HMM tests skip on Windows (hmmlearn requires C++ build tools; passes in CI on Linux). +17 tests in Sprint 6 Phase 13 (four Render bugfixes): `test_render_bugfixes.py` — migration 006 import + body check (2), `_safe_json_parse` truncation + fence + prose tolerance (8), Haiku fallback `max_tokens` floor (2), Grok xAI timeout + body-shape alignment with contrarian + 4xx body-preview logging (3), `_append_incremental_daily` FRED `.iloc[:, 0]` access pattern (2). |
