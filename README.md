...
- **Fama-French is three-factor, not Carhart four-factor.** `ff_factors_monthly` holds Mkt-RF / SMB / HML / RF only — momentum is not in the dataset, so factor-loading regressions are three-factor.
- **Turnover column** on the Dashboard strategy table is `avg_monthly_turnover` annualised (×12) — a rebalance-frequency proxy, not true weight-change portfolio turnover.
