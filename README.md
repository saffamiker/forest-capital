...
| Layer | Tests | Notes |
|-------|-------|-------|
| Backend (pytest) | 951 passed, 15 skipped | HMM tests skip on Windows (hmmlearn requires C++ build tools; passes in CI on Linux). +6 tests in `test_optimize_endpoint.py` for the Efficient Frontier fix: `/api/optimize/weights` now returns the structured `{frontier_points, portfolio_points, max_sharpe_point, min_variance_point}` payload in both real and mock paths (a flat list left the chart blank). `portfolio_points` are read from the latest `strategy_results_cache` row — no `get_full_history()` / `run_all_strategies()` recompute on an optimize request. `_build_efficient_frontier()` reshape is unit-tested for the `return`→`expected_return` rename, empty sweep, and malformed-point tolerance. |
