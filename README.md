...
| Layer | Tests | Notes |
|-------|-------|-------|
| Backend (pytest) | 847 passed, 10 skipped | HMM tests skip on Windows (hmmlearn requires C++ build tools; passes in CI on Linux). +27 tests in Sprint 6 Phase 11 (Bob's section editor + persona endpoint): 7 `TestAnalyticalAppendixEndpoint`, 6 `TestExecutiveBriefEndpoint`, 2 `TestManifestNewGenerators`, 5 `TestAgentPersonasEndpoint`, 7 `TestSectionDocumentEndpoints` (section-doc draft / regenerate / export). |
| Frontend (Vitest) | 171 passed | +30 in Phase 11: 14 `section-editor.test.tsx` (documentsStore CRUD + SectionEditor rendering + per-section affordances), 11 `persona-modal.test.tsx` (three-tab modal, glossaryStore.loadPersona idempotency, close affordances), 7 in `StrategyCard.test.tsx` for the new ExplainableText metric-label wraps (Sharpe Ratio / CAGR / Max Drawdown / Volatility / CV Stability Score / Tier 1 / Analyst-mode plain-text fallback). |
