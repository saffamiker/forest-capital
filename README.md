...
The platform has **two access tiers**, so it can be shared with guests — Dr. Panttser, reviewers — without exposing the action features.

- **Explore (any authenticated user).** All analytics, dashboards and charts; the AI council — ask a question, use the inline ⓘ explainers and Data Explain; and the Team Activity read view. A guest sees a one-time welcome banner setting these expectations.
- **Act (project team only — `PROJECT_TEAM_EMAILS`: Michael, Bob, Molly).** Academic document upload/delete, all four export endpoints (academic package and the three generated documents), Academic Review, the guided test runner, and the Settings modifications (brand switcher, Retake Site Tour, Testing Mode).

Frontend: `TeamGate` wraps every action element — a guest sees it muted with a lock icon and a tooltip, or hidden entirely. Backend: the `require_team_member` dependency returns 403 for a non-team authenticated user on every gated endpoint. The **AI council question is deliberately open** — it is read-only, scope-guarded and rate-limited, and letting a guest interrogate the analysis is the whole point of sharing the platform. See `CLAUDE.md` → Two Access Tiers.
