...
class TestMidpointPaperAppendix:
    """build_midpoint_paper renders the Audit Disclosure Appendix at the
    end of the body. The two disclosure tables surface when the bundle
    carries acknowledged warnings; without an audit the appendix
    renders a single explanatory paragraph instead."""
    ...
    def test_renders_appendix_heading(self):
        from tools.academic_docx import build_midpoint_paper

        blob = build_midpoint_paper(self._data(), {})
        text = _docx_text(blob)
        assert "Audit Disclosure Appendix" in text

    def test_renders_statistical_acknowledgement_with_metadata(self):
        from tools.academic_docx import build_midpoint_paper

        blob = build_midpoint_paper(self._data(), {})
        text = _docx_text(blob)
        # Reviewer email, date, and note all surface in the appendix.
        assert "bob@queens.edu" in text
        assert "2026-05-25" in text
        assert "ACKDISCLOSURETOKEN" in text

    def test_renders_intentional_disclosure_with_metadata(self):
        from tools.academic_docx import build_midpoint_paper

        blob = build_midpoint_paper(self._data(), {})
        text = _docx_text(blob)
        assert "P03" in text
        assert "ruurdsm@queens.edu" in text
        assert "INTENTIONALDISCLOSURETOKEN" in text

    def test_no_audit_on_record_renders_explanation(self):
        from tools.academic_docx import build_midpoint_paper

        blob = build_midpoint_paper(
            self._data(audit_disclosures=_empty_disclosures()), {})
        text = _docx_text(blob)
        assert "Audit Disclosure Appendix" in text
        # Empty-state paragraph appears in place of the tables.
        assert "No platform audit was on record" in text

    def test_clean_audit_with_no_warnings_renders_both_no_records_lines(
        self,
    ):
        from tools.audit_summary import gather_audit_disclosures  # noqa
        from tools.academic_docx import build_midpoint_paper

        clean = _disclosures()
        clean["statistical"]["acknowledged"] = []
        clean["methodology"]["intentional"] = []
        clean["statistical"]["warnings"] = 0
        clean["methodology"]["warnings"] = 0
        blob = build_midpoint_paper(
            self._data(audit_disclosures=clean), {})
        text = _docx_text(blob)
        assert "statistical audit surfaced no warnings" in text
        assert "No methodology checks were marked as intentional" in text
