...
def make_random_history(n_months: int = 60, seed: int = 42) -> dict:
    """
    History dict with random returns — used where we need non-zero variance
    (e.g. Sharpe ratio, risk parity). Fixed seed ensures reproducibility.
    """
    np.random.seed(seed)
    idx_m = pd.date_range("2020-01-31", periods=n_months, freq="ME")
    n_daily = n_months * 21
...
# ── TEST 5: Equal weight assignment ───────────────────────────────────────────
...
def test_risk_parity_weights_sum_to_one():
    ...
    result = run_risk_parity(make_random_history())
...
# ── TEST 7: run_all_strategies return type ────────────────────────────────────
...
def test_run_all_strategies_returns_dict():
    ...
    result = run_all_strategies(make_random_history())
...
# ── TEST 8: Monthly return additivity ─────────────────────────────────────────
