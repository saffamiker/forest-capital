 ...
 * interactive submission checklist (seven items as of June 23 2026
 * -- gained the Brief Review row when the per-document review
 * surfaces landed) that toggles on click and unlocks a green
 * confirmation banner when ALL items are checked.
 *
...
describe('BriefWorkflowModal -- interactive submission checklist', () => {
  ...
  it('renders all seven checkboxes unchecked on modal open', () => {
    render(
      <BriefWorkflowModal open={true} onClose={() => undefined} />)
    for (let i = 0; i < 7; i++) {
  ...
  it('completion banner does NOT render until all seven are checked',
    () => {
      render(
        <BriefWorkflowModal open={true} onClose={() => undefined} />)
      // Check six out of seven.
      for (let i = 0; i < 6; i++) {
  ...
  it('completion banner renders when all seven are checked', () => {
    render(
      <BriefWorkflowModal open={true} onClose={() => undefined} />)
    for (let i = 0; i < 7; i++) {
      fireEvent.click(screen.getByTestId(`brief-checklist-item-${i}`))
