...
# ── Hash-keyed regime-signals snapshots (June 27 2026) ──────────────
#
# Document generators under submission freeze read regime signals via
# the hash-aware path -- if the freeze hash has no snapshot, the
# generator sets live_signals=None and watchpoint tokens render
# em-dash (NOT the deprecated [DATA PENDING] placeholder). This pair
# of helpers + migration 065 supplies the read/write surface.
#
# Snapshot is written once by submission_freeze.set_freeze_config(
# active=True, freeze_hash=...) so the active freeze always has a
# regime-signals snapshot keyed to its hash. Subsequent set_freeze_
# config calls are no-ops if a snapshot already exists for the hash
# (idempotent -- the snapshot is a permanent point-in-time record
# that must not be silently overwritten by post-freeze drift).
#
# Schema: explicit columns mirroring regime_signals_cache (minus
# TTL fields) so operator INSERTs are simple SQL + direct column
# queries work. See migration 065.

# Columns the snapshot row preserves (everything from the live
# regime_signals_cache row that the substitution table consumes).
_REGIME_SNAPSHOT_COLUMNS: tuple[str, ...] = (
    "threshold_regime", "hmm_regime", "hmm_probabilities",
    "regimes_agree", "vix_level", "yield_curve_slope",
    "credit_spread", "equity_trend",
    "pre_2022_avg_correlation", "post_2022_avg_correlation",
)
...
async def snapshot_regime_signals_for_hash(
    data_hash: str,
    signals_row: dict[str, Any] | None = None,
) -> bool:
    """Snapshot the live regime_signals_cache row into the hash-keyed
    snapshot table.

    signals_row -- when supplied, used verbatim (the caller already
    loaded the live signals and wants to capture them; this avoids
    a redundant cache read inside a transaction). When None, loads
    the current live row via get_regime_cache().

    Returns True when a snapshot was written; False on (a) snapshot
    already exists for the hash (idempotent no-op), (b) missing
    live cache (nothing to snapshot), or (c) DB write error.

    Idempotent: a row already present for data_hash is preserved
    untouched -- the snapshot is a permanent point-in-time record
    of the freeze, and a re-activation of the same freeze must not
    silently overwrite it with post-freeze drift. The operator can
    explicitly clear the row first if a fresh capture is needed."""
    if not _DB_AVAILABLE or not data_hash:
        return False
    payload = signals_row
    if payload is None:
        payload = await get_regime_cache()
    if not isinstance(payload, dict):
        log.warning(
            "regime_snapshot_skipped_no_live_signals",
            data_hash=data_hash[:8],
            note=("regime_signals_cache had no row to snapshot; "
                  "the freeze hash will resolve to live_signals=None "
                  "on document export"))
        return False
    try:
        from sqlalchemy import text
        now = datetime.now(timezone.utc)
        async with AsyncSessionLocal() as session:  # type: ignore[union-attr]
            # Idempotency: ON CONFLICT DO NOTHING means a second
            # set_freeze_config(active=True, freeze_hash=<same>) does
            # not overwrite the existing snapshot. The operator clears
            # the row first if they need a fresh capture.
            hp_raw = payload.get("hmm_probabilities")
            hp_json = (
                json.dumps(hp_raw, default=str)
                if hp_raw is not None else None)
            await session.execute(
                text(
                    "INSERT INTO regime_signals_snapshots ("
                    ...
                    "VALUES ("
                    "  :data_hash, :threshold_regime, :hmm_regime, "
                    "  CAST(:hmm_probabilities AS JSONB), "
                    "  :regimes_agree, "
                    "  :vix_level, :yield_curve_slope, "
                    "  :credit_spread, :equity_trend, "
                    "  :pre_2022_avg_correlation, "
                ...
                {
                    "data_hash": data_hash,
                    "threshold_regime": payload.get("threshold_regime"),
                    "hmm_regime": payload.get("hmm_regime"),
                    "hmm_probabilities": hp_json,
                    "regimes_agree": payload.get("regimes_agree"),
                    "vix_level": payload.get("vix_level"),
                    "yield_curve_slope": payload.get(
                        "yield_curve_slope"),
                    "credit_spread": payload.get("credit_spread"),
                    "equity_trend": payload.get("equity_trend"),
                    "pre_2022_avg_correlation": payload.get(
                        "pre_2022_avg_correlation"),
                    "post_2022_avg_correlation": payload.get(
                        "post_2022_avg_correlation"),
                    "snapshotted_at": now,
                },
            )
            await session.commit()
        log.info(
            "regime_snapshot_attempted",
            data_hash=data_hash[:8],
            snapshotted_at=now.isoformat(),
            note="ON CONFLICT DO NOTHING; pre-existing snapshot left intact")
        return True
    except Exception as exc:  # noqa: BLE001
        log.warning(
            "regime_snapshot_write_error",
            data_hash=data_hash[:8], error=str(exc))
        return False


async def get_regime_snapshot_for_hash(
    data_hash: str,
) -> dict[str, Any] | None:
    """Returns the regime_signals snapshot for the given freeze hash,
    or None when no snapshot exists. Document generators under freeze
    fall back to live_signals=None on a None return -- watchpoint
    tokens then render em-dash to signal 'no data captured for this
    hash' (NOT [DATA PENDING], which has been retired).

    The returned dict mirrors the shape get_regime_cache() returns
    (same keys -- threshold_regime, hmm_regime, hmm_probabilities,
    regimes_agree, vix_level, yield_curve_slope, credit_spread,
    equity_trend, pre_2022_avg_correlation, post_2022_avg_correlation)
    plus an 'as_of' ISO timestamp derived from snapshotted_at, so
    every caller that reads live_signals works with snapshots
    transparently."""
    if not _DB_AVAILABLE or not data_hash:
        return None
    try:
        ...
        async with AsyncSessionLocal() as session:  # type: ignore[union-attr]
            row = await session.execute(
                text(
                    "SELECT threshold_regime, hmm_regime, "
                    "       hmm_probabilities, regimes_agree, "
                    "       vix_level, yield_curve_slope, "
                    "       credit_spread, equity_trend, "
                    "       pre_2022_avg_correlation, "
                    "       post_2022_avg_correlation, "
                    "       snapshotted_at "
                    "FROM regime_signals_snapshots "
                    "WHERE data_hash = :h LIMIT 1"
                ),
                {"h": data_hash})
            r = row.fetchone()
            if r is None:
            ...
            if isinstance(hp_raw, str):
                ...
                hp_value = hp_raw
            return {
                "threshold_regime": r[0],
                "hmm_regime": r[1],
                "hmm_probabilities": hp_value,
                "regimes_agree": r[3],
                "vix_level": r[4],
                "yield_curve_slope": r[5],
                "credit_spread": r[6],
                "equity_trend": r[7],
                "pre_2022_avg_correlation": r[8],
                "post_2022_avg_correlation": r[9],
                "as_of": (
                    r[10].isoformat() if r[10] else None),
            }
    except Exception as exc:  # noqa: BLE001
        log.warning(
            "regime_snapshot_read_error",
            data_hash=data_hash[:8], error=str(exc))
        return None
