        ...
        specs = [
            {"key": "executive_summary", "available": avail,
             ...
             "task": (
                 ...
                 "adjusted basis over the out-of-sample window "
                 ...
                 "(regime-conditional blend) vs {{OOS_SHARPE_BENCHMARK}} "
                 "(benchmark); maximum drawdown "
                 "{{REGIME_SWITCHING_MAX_DD}} (Regime Switching) vs "
                 "{{BENCHMARK_MAX_DD}} (benchmark); the regime-conditional "
                 "construction held the bond sleeve through the 2022 "
                 "equity drawdown.\n\n"
                 "STRATEGY NAMING -- non-negotiable distinction: 'Regime "
                 "Switching' is the underlying strategy (label for the "
                 "full-period Sharpe + drawdown); 'regime-conditional "
                 "blend' is the HMM-posterior-weighted portfolio (label "
                 "for the OOS Sharpe + the recommendation).\n\n"
            ...
            {"key": "key_findings", "available": avail, "pending": pending,
             ...
             "task": (
                 ...
                 "{{REGIME_SWITCHING_MAX_DD}} (Regime Switching)\n"
                 "  - OOS Sharpe {{OOS_SHARPE_BLEND}} "
                 "(regime-conditional blend) vs "
                 "{{OOS_SHARPE_BENCHMARK}} (benchmark)\n"
                 "  - {{OOS_WINDOW_MONTHS}}-month out-of-sample "
                 "window (January 2022 through May 2026)\n\n"
                 "STRATEGY NAMING -- non-negotiable distinction:\n"
                 "  - 'Regime Switching' is the underlying strategy "
                 "(REGIME_SWITCHING in the cache). Use this label "
                 "when citing the full-period Sharpe, the max "
                 "drawdown ({{REGIME_SWITCHING_MAX_DD}}), the "
                 "recovery months, or any other full-period "
                 "metric in the per-strategy comparison.\n"
                 "  - 'regime-conditional blend' is the HMM-"
                 "posterior-weighted portfolio across strategies. "
                 "Use this label when citing the out-of-sample "
                 "Sharpe ({{OOS_SHARPE_BLEND}}), the OOS validation "
                 "evidence, or the recommendation itself.\n"
                 "  - Wrong: 'Regime Switching achieved an OOS "
                 "Sharpe of {{OOS_SHARPE_BLEND}}'. Right: 'the "
                 "regime-conditional blend achieved an OOS Sharpe "
                 "of {{OOS_SHARPE_BLEND}}'.\n\n"
                 "IMPORTANT: The token {{OOS_SHARPE_IMPROVEMENT_PCT}} "
                 "already resolves to a complete formatted string "
                 "including the + prefix and % suffix (e.g. '+85%'). "
            ...
            {"key": "limitations", "available": True,
             ...
             "task": (
                 ...
                 "out-of-sample data (January 2022 through May 2026) "
                 "is approximately {{OOS_WINDOW_PCT_OF_STUDY}}% of "
                 "the full study window. "
            ...
            {"key": "final_recommendations", "available": True,
             ...
             "task": (
                 ...
                 "in the environment defined by the 2022 correlation "
                 "regime shift.'\n\n"
...
_APPENDIX_NARRATIVE_TASKS = {
    ...
    "appendix_b": (
        ...
        "for direct comparison. Close with a sentence clarifying that "
        "the REGIME_SWITCHING row in this table reports the UNDERLYING "
        "strategy's full-period figures (Sharpe, drawdown, recovery), "
        "while the regime-conditional blend's out-of-sample Sharpe "
        "(see Section G for the OOS validation) reflects the HMM-"
        "posterior-weighted combination across strategies and is a "
        "DIFFERENT object from the REGIME_SWITCHING strategy itself. "
        "Plain academic prose. No em dashes."),
    "appendix_c": (
...
_NUMERIC_PLACEHOLDER_GUIDE = (
    ...
    "PROHIBITED STRATEGY TOKENS (non-negotiable, June 29 2026 -- "
    "draft-88 fix). The brief / appendix / deck submission scope "
    "is BENCHMARK + CLASSIC_60_40 + REGIME_SWITCHING only. NEVER "
    "emit tokens for strategies outside the submission scope: "
    "{{MIN_VARIANCE_*}}, {{RISK_PARITY_*}}, "
    "{{BLACK_LITTERMAN_*}}, {{EQUAL_WEIGHT_*}}, "
    "{{VOL_TARGETING_*}}, {{MOMENTUM_ROTATION_*}}, "
    "{{MAX_SHARPE_ROLLING_*}} are HALLUCINATIONS -- the "
    "substitution table contains no entries for them under the "
    "three-strategy scope, and they will render as literal "
    "{{...}} text in the exported brief. If you find yourself "
    "wanting to discuss those strategies, REWRITE the sentence "
    "to reference only the three in-scope strategies; do NOT "
    "invent a token.\n"
    "ONLY THESE TOKEN PREFIXES ARE PERMITTED:\n"
    "  {{BENCHMARK_*}}, {{CLASSIC_6040_*}}, "
    "{{REGIME_SWITCHING_*}}, {{OOS_SHARPE_*}}, {{STUDY_*}}, "
    "{{OOS_WINDOW*}}, {{REBALANCE_*}}, {{CURRENT_*}}, "
    "{{PRE_2022_*}}, {{POST_2022_*}}, {{NET_SHARPE_*}}, "
    "{{REGIME_*}}, {{VIX_*}}, {{YIELD_*}}, {{CREDIT_*}}, "
    "{{EQUITY_*}}, {{ESS_*}}, {{DATA_HASH}}, "
    "{{N_STRATEGIES}}, {{PLAY_BY_PLAY_*}}, "
    "{{DD_REDUCTION_*}}, {{BH_SIGNIFICANCE_*}}, "
    "{{BOOTSTRAP_*}}.\n\n"
    ...
    "(resolves to e.g. '+85%'); do NOT add surrounding + or % "
    ...
    "ALREADY INCLUDED (resolves to e.g. '32 months'). NEVER write "
    "'months' after this token -- the unit is already there. "
    "Wrong: '{{REGIME_SWITCHING_RECOVERY_MONTHS}} months'. Right: "
    ...
    "  {{BENCHMARK_RECOVERY_MONTHS}} -- benchmark recovery WITH "
    "UNITS INCLUDED (e.g. '71 months'). NEVER write 'months' "
    "after this token. Wrong: '{{BENCHMARK_RECOVERY_MONTHS}} "
    "months' (renders '71 months months'). Right: "
    "'{{BENCHMARK_RECOVERY_MONTHS}}'.\n"
    "  {{CLASSIC_6040_RECOVERY}} -- 60/40 recovery in months (number "
    "only). Write 'months' after in your prose.\n"
    "  {{CLASSIC_6040_RECOVERY_MONTHS}} -- 60/40 recovery WITH "
    "UNITS INCLUDED. NEVER write 'months' after this token.\n"
...
_APPENDIX_NUMERIC_PLACEHOLDER_GUIDE_EXTENSION = (
    ...
    "\n"
    "PROHIBITED STRATEGY TOKENS (non-negotiable, June 29 2026 -- "
    "draft-88 fix). NEVER emit tokens for strategies outside the "
    "submission scope. The substitution table does NOT contain "
    "entries for MIN_VARIANCE, RISK_PARITY, BLACK_LITTERMAN, "
    "EQUAL_WEIGHT, VOL_TARGETING, MOMENTUM_ROTATION, or "
    "MAX_SHARPE_ROLLING under the three-strategy submission scope. "
    "Tokens like {{MIN_VARIANCE_POST2022_SHARPE}}, "
    "{{RISK_PARITY_RECOVERY_MONTHS}}, {{BLACK_LITTERMAN_RECOVERY_MONTHS}}, "
    "{{EQUAL_WEIGHT_RECOVERY_MONTHS}}, {{VOL_TARGETING_RECOVERY_MONTHS}}, "
    "{{MOMENTUM_ROTATION_RECOVERY_MONTHS}}, "
    "{{MAX_SHARPE_ROLLING_RECOVERY_MONTHS}} are HALLUCINATIONS -- "
    "they will not resolve and will render literally as {{...}} "
    "text in the exported document.\n"
    "\n"
    "ONLY THESE TOKEN PREFIXES ARE PERMITTED IN BRIEF / APPENDIX / "
    "DECK PROSE:\n"
    "  {{BENCHMARK_*}}, {{CLASSIC_6040_*}}, {{REGIME_SWITCHING_*}}, "
    "  {{OOS_SHARPE_*}}, {{STUDY_*}}, {{OOS_WINDOW*}}, "
    "  {{REBALANCE_*}}, {{CURRENT_*}}, {{PRE_2022_*}}, "
    "  {{POST_2022_*}}, {{NET_SHARPE_*}}, {{REGIME_*}}, "
    "  {{VIX_*}}, {{YIELD_*}}, {{CREDIT_*}}, {{EQUITY_*}}, "
    "  {{ESS_*}}, {{DATA_HASH}}, {{N_STRATEGIES}}, "
    "  {{PLAY_BY_PLAY_*}}, {{DD_REDUCTION_*}}, "
    "  {{BH_SIGNIFICANCE_*}}, {{BOOTSTRAP_*}}.\n"
    "Any other strategy-prefixed token is a hallucination. If you "
    "find yourself wanting to discuss MIN_VARIANCE or any other out-"
    "of-scope strategy, REWRITE the sentence to reference only the "
    "three in-scope strategies; do NOT invent a token.\n"
    "\n"
